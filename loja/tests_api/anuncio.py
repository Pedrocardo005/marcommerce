...
class AnuncioTestCase(BaseRegistredUser):
    ...
    def test_get_anuncio(self):
        ...
        self.assertEqual(response["email_anunciante"], "teste@teste.com")
    ...
    def test_get_anuncios_by_categoria(self):
        ...
        url = reverse(
            "loja.anuncios-categoria", kwargs={"pk": subcategoria.categoria.pk}
        )
    ...
    def test_change_status(self):
        ...
        url_change_status = reverse(
            "loja.anuncio-change-status", kwargs={"pk": anuncio_id}
        )
    ...
    def test_favorite_anuncio(self):
        ...
        data = {"id_anuncio": anuncio.pk}
        response = self.client.post(
            url_favorite_anuncio,
            data,
            headers={"Authorization": f"Bearer {self.token}"},
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        url_delete_favorite_anuncio = reverse(
            "loja.delete-favorite-anuncio", kwargs={"pk": id_favorito}
        )
        response = self.client.delete(
            url_delete_favorite_anuncio,
            headers={"Authorization": f"Bearer {self.token}"},
        )
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
